...
from my_library.mask import get_Gaussian2D_mask
...
def my_bilateral(src, msize, sigma, sigma_r, pos_x, pos_y, pad_type='zero'):
    ...
    ####################################################################################################\
    (h, w) = src.shape
    img_pad = my_padding(src, (msize // 2, msize // 2))
    dst = np.zeros((h, w))
    ...
    for i in range(h):
        print('\r%d / %d ...' % (i, h), end="")
        for j in range(w):
            src_part = np.full((msize, msize), src[i][j])
            pad_part = img_pad[i:i + msize, j:j + msize]
            mask = np.exp(-((x ** 2) / (2 * (sigma ** 2))) - ((y ** 2) / (2 * (sigma ** 2)))) * \
                   np.exp(-(src_part - pad_part) ** 2 / (2 * (sigma_r ** 2)))
            mask /= np.sum(mask)
            ...
            if i == pos_y and j == pos_x:
                ...
                cv2.imshow('img', img)
            target = img_pad[i:i + msize, j:j + msize]
...
if __name__ == '__main__':
    ...
    np.random.seed(seed=100)  # seed값 바꾸지 말것
    ...
    gaus2D = get_Gaussian2D_mask(5, sigma=10)
